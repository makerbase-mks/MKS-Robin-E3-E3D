...
void mks_test(){
	...
	if(digitalRead(X_STOP_PIN)==0 && digitalRead(Y_STOP_PIN)==0
		&& digitalRead(Z_MAX_PIN)==0 && digitalRead(Z_MIN_PIN)==0 &&
   digitalRead(PB10)==0){
		//ui.set_status_P("END OK",1);
    SERIAL_ECHO_MSG("IO OK");
	}
	else{
		//ui.set_status_P("END ERR",1);
		mks_test_beeper();
	}
}
#endif
